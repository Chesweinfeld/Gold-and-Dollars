...
def render(key, r, value, geo, tiles, quads, moved, n_lattice, bidx,
    ...
    if key == "us":
        ...
        # Every cut offers the country and the largest few, itself excepted:
        # the row is how a reader moves between them without going back
        # first.  It used to be `key in dict(CUTS)`, which is the five metros
        # the national map links to -- so those five could get home and the
        # other seventy-six could not.  A reader who searched their way into
        # Houston had no way out of it.
        nav = ('<span class="sp"></span>'
               '<a href="../index.html">&larr; the whole country</a>'
    ...
    html = _HTML.format(
        head=head, nav=nav,
        # The oldest convention on the web, and the one a reader reaches for
        # before they read a control row: the way back is at the top left.
        home=('<a class="home" href="../index.html" '
              'title="the whole country">&#8598;</a>'
              if r["window"] is not None else ""),
...
/* The map is drawn at whatever shape its ground is, and a metro's ground can
   be tall: Los Angeles runs 229 km north to south against 133 km across, so a
   full-width frame at that ratio stands 1,900 px high and the reader meets the
   top third of a map and no way of knowing there is more.  The width is
   therefore taken from the height that is left on the screen, and the frame
   keeps its ratio rather than stretching to fill -- a cartogram stretched is a
   cartogram lying.  min() takes the container's width when that is the tighter
   of the two, which is what happens on a phone and on the wide national map;
   the 340 px floor is for a window too short for either to be much use. */
#stage {{ position:relative; aspect-ratio:{W} / {H}; margin:0 auto;
  width:min(100%, max(300px, calc((100vh - 240px) * {W} / {H})));
...
.nn {{ font:500 10.5px/1 ui-sans-serif,system-ui,-apple-system,"Segoe UI",
  ...
  user-select:none; }}
a.home {{ text-decoration:none; color:var(--muted); margin-right:.4em;
  font-weight:400; }}
