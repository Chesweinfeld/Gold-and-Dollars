...
// k=1 is the whole map at the width of the frame.  Below it the map is
// drawn smaller than its frame, which is the only way to see a tall cut
// -- Los Angeles is 1.7 times as high as it is wide -- without scrolling
// the page to follow it.
const MINK=0.2;
...
function zoomAbout(k,mx,my) {{
  k=Math.min(Math.max(k,MINK),MAXK);
  const g=k/view.k;
...
stage.addEventListener('wheel',e=>{{
  ...
  const k=Math.min(Math.max(view.k*f,MINK),MAXK);
  const g=k/view.k;
  view.ox=mx-(mx-view.ox)*g; view.oy=my-(my-view.oy)*g;
  view.k=k; clampView(); draw();
}},{{passive:false}});
function clampView() {{
  ...
  // Zoomed in, the frame stays covered: the edge of the map cannot be
  // dragged into the middle of it.  Zoomed out past the fit there is no edge
  // to hold, so the map is centred in the frame instead of pinned to a corner.
  // Centred across, but held against the top rather than the middle: on a
  // tall cut the frame is taller than the window, so a map centred in it
  // would be zoomed out to somewhere the reader cannot see without scrolling
  // the page after it.  At the top it is where they are already looking.
  const fw=W*view.k, fh=H*view.k;
  view.ox = fw<=W ? (W-fw)/2 : Math.min(0,Math.max(W-fw,view.ox));
  view.oy = fh<=H ? 0 : Math.min(0,Math.max(H-fh,view.oy));
}}
